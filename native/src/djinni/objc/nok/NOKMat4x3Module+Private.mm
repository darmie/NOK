...
#import "NOKMat4x3Module+Private.h"
...
@interface NOKMat4x3Module ()

- (id)initWithCpp:(const std::shared_ptr<::nok::Mat4x3Module>&)cppRef;

@end

@implementation NOKMat4x3Module {
    ::djinni::CppProxyCache::Handle<std::shared_ptr<::nok::Mat4x3Module>> _cppRefHandle;
}
...
+ (nullable NOKMat4x3Module *)create:(nullable id<RVReactBridge>)bridge {
    try {
        auto objcpp_result_ = ::nok::Mat4x3Module::create(::djinni_generated::ReactBridge::toCpp(bridge));
        return ::djinni_generated::Mat4x3Module::fromCpp(objcpp_result_);
...
{
    return @{
        @"width": @(NOKMat4x3ModuleWidth),
        @"height": @(NOKMat4x3ModuleHeight),
    };
}

namespace djinni_generated {

auto Mat4x3Module::toCpp(ObjcType objc) -> CppType
{
    if (!objc) {
        return nullptr;
    }
...
auto Mat4x3Module::fromCppOpt(const CppOptType& cpp) -> ObjcType
{
    if (!cpp) {
        return nil;
    }
    return ::djinni::get_cpp_proxy<NOKMat4x3Module>(cpp);
}
