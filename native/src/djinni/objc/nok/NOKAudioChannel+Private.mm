...
@interface NOKAudioChannel ()

- (id)initWithCpp:(const std::shared_ptr<::nok::AudioChannel>&)cppRef;

@end

@implementation NOKAudioChannel {
    ::djinni::CppProxyCache::Handle<std::shared_ptr<::nok::AudioChannel>> _cppRefHandle;
}
...
+ (nullable NOKAudioChannel *)create:(BOOL)looping {
    try {
        auto objcpp_result_ = ::nok::AudioChannel::create(::djinni::Bool::toCpp(looping));
...
auto AudioChannel::toCpp(ObjcType objc) -> CppType
{
    if (!objc) {
        return nullptr;
    }
...
auto AudioChannel::fromCppOpt(const CppOptType& cpp) -> ObjcType
{
    if (!cpp) {
        return nil;
    }
    return ::djinni::get_cpp_proxy<NOKAudioChannel>(cpp);
}
