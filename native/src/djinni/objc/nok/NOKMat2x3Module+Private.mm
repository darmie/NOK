...
#import "NOKMat2x3Module+Private.h"
...
@interface NOKMat2x3Module ()

- (id)initWithCpp:(const std::shared_ptr<::nok::Mat2x3Module>&)cppRef;

@end

@implementation NOKMat2x3Module {
    ::djinni::CppProxyCache::Handle<std::shared_ptr<::nok::Mat2x3Module>> _cppRefHandle;
}
...
+ (nullable NOKMat2x3Module *)create:(nullable id<RVReactBridge>)bridge {
    try {
        auto objcpp_result_ = ::nok::Mat2x3Module::create(::djinni_generated::ReactBridge::toCpp(bridge));
        return ::djinni_generated::Mat2x3Module::fromCpp(objcpp_result_);
...
{
    return @{
        @"width": @(NOKMat2x3ModuleWidth),
        @"height": @(NOKMat2x3ModuleHeight),
    };
}

namespace djinni_generated {

auto Mat2x3Module::toCpp(ObjcType objc) -> CppType
{
    if (!objc) {
        return nullptr;
    }
...
auto Mat2x3Module::fromCppOpt(const CppOptType& cpp) -> ObjcType
{
    if (!cpp) {
        return nil;
    }
    return ::djinni::get_cpp_proxy<NOKMat2x3Module>(cpp);
}
