...
#import "NOKVec2Module+Private.h"
#import "NOKVec2Module.h"
...
@interface NOKVec2Module ()

- (id)initWithCpp:(const std::shared_ptr<::nok::Vec2Module>&)cppRef;

@end

@implementation NOKVec2Module {
    ::djinni::CppProxyCache::Handle<std::shared_ptr<::nok::Vec2Module>> _cppRefHandle;
}
...
+ (nullable NOKVec2Module *)create:(nullable id<RVReactBridge>)bridge {
    try {
        auto objcpp_result_ = ::nok::Vec2Module::create(::djinni_generated::ReactBridge::toCpp(bridge));
        return ::djinni_generated::Vec2Module::fromCpp(objcpp_result_);
...
RCT_REMAP_METHOD(getXy, getXy:(nonnull RCTPromiseResolveBlock)resolver
                     rejecter:(nonnull RCTPromiseRejectBlock)rejecter) {
    try {
        _cppRefHandle.get()->get_xy(::djinni_generated::JavascriptPromise::toCpp([RVReactDjinni wrapPromiseWithResolver:resolver rejecter:rejecter]));
    } DJINNI_TRANSLATE_EXCEPTIONS()
}

RCT_EXPORT_METHOD(set:(double)i
                    v:(double)v) {
    try {
...
auto Vec2Module::toCpp(ObjcType objc) -> CppType
{
    if (!objc) {
        return nullptr;
    }
...
auto Vec2Module::fromCppOpt(const CppOptType& cpp) -> ObjcType
{
    if (!cpp) {
        return nil;
    }
    return ::djinni::get_cpp_proxy<NOKVec2Module>(cpp);
}
